...
# ==========================================
# INTERFACE APPLICATION LAYOUT (HEADER & REAL-TIME CLOCK)
# ==========================================
...
nama_file_gambar = "D:/speedhome-analytics/image_023cbd.jpg" 
...
# PERUBAHAN UTAMA: Jam Real-Time yang dioptimalkan agar terhindar dari bug 'Memuat terus' akibat re-run Streamlit
st.markdown("""
<div class="realtime-clock-container">
    <span style="margin-right: 8px;">🕒</span>
    <span id="live-clock-optimized">Menghubungkan Waktu...</span>
</div>

<script>
    (function() {
        function startLiveClock() {
            const clockElement = document.getElementById('live-clock-optimized');
            if (!clockElement) {
                // Jika elemen belum siap di DOM, coba lagi dalam 50ms
                setTimeout(startLiveClock, 50);
                return;
            }
            ...
            function tick() {
                ...
                const stringTanggal = now.toLocaleDateString('id-ID', opsiTanggal);
                const stringJam = now.toLocaleTimeString('id-ID', { hour12: false });
                
                if (document.getElementById('live-clock-optimized')) {
                    document.getElementById('live-clock-optimized').innerHTML = stringTanggal + " | " + stringJam + " WIB";
                }
            }
            
            // Eksekusi langsung saat dipanggil pertama kali
            tick();
            // Atur interval per detik
            setInterval(tick, 1000);
        }
        
        // Jalankan fungsi secara anonim dan langsung
        startLiveClock();
    })();
</script>
""", unsafe_allow_html=True)
...
<div class="animate-fade hero-header">
    <div class="header-content">
        <h1 class="main-title-custom">🏢 Property Price Intelligence System</h1>
        <p class="subtitle-custom">CEO Office Strategic Decision Tool — Real-time Analytics Dashboard for SPEEDHOME Malaysia</p>
    </div>
</div>
""", unsafe_allow_html=True)
